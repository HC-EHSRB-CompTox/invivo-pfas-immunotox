...
data_spleen <- data_spleen[ , !(names(data_spleen) %in% c("B220..F4.80.","B220.macs","CD4.CD25.CD69.ve","CD4.CD69","CD4.DP","CD4.DCs","CD8.CD69","CD8dcs", "NK"))]
...
heatmap_with_sig$Marker <- factor(heatmap_with_sig$Marker, levels = c("T.cells","CD4.T.cells","CD8", "B.Cells","DCs"))
...
dose_levels <- c("Naive", "Vehicle", "0.166", "0.5", "1", "1.5")
marker_levels <- c("T.cells","CD4.T.cells","CD8", "B.Cells","DCs")
...
ggsave(
  filename = "../invivo-pfas-immunotox/R input 56d/plots/blood_heatmap_NOSIG.png",
  plot = heatmap_blood,
  width = plot_width,
  height = plot_height,
  dpi = 300,
...
data_thymus <- data_thymus[ , !(names(data_thymus) %in% c("CD3.CD69.", "CD3.CD69..3", "DP.CD69", "DN.CD3"))]
...
sig_for_heatmap <- significance_df_non_parametric %>%
  ...
  filter(!Marker %in% c("Macrophages", "B.Cells", "Dendritic.Cells", "DN.DP")) %>%
  dplyr::select(Sex, PFAS, Marker, Dose, significance)
...
plot <- ggplot(data_tdar, aes(x = interaction(Dose, Sex), y = Value)) +
  ...
  labs(
    title = "Norma lized Response by Dose and Sex",
    x = "Dose and Sex",
    y = "Value"
  ) +
...
############################################################################################
#drc
create_drc_plot <- function(df, chemical, file_name, color_names) {
  data_clean <- df %>%
    filter(PFAS == chemical, Marker %in% c("ALP", "ALT", "TGL", "CA")) %>%
    mutate(
  ...
  avg_measured <- data_clean %>%
    group_by(Dose_num) %>%
    summarize(
      Mean_Measured = mean(.data[[chemical]], na.rm = TRUE),
      .groups = "drop"
    ) %>%
  ...
  models_measured <- nested %>%
    ...
    filter(!map_lgl(pred, is.null)) %>%    
    unnest(pred) %>%
    mutate(Source = paste("Measured", chemical))
  
  models_combined <- bind_rows(models_nominal, models_measured)
  
  raw_nominal  <- nested %>% unnest(data) %>% mutate(x = Cumulative_Dose_adj, Source = "Nominal Dose")
  raw_measured <- nested %>% unnest(data) %>% mutate(x = Measured_adj, Source = paste("Measured", chemical))
  raw_combined <- bind_rows(raw_nominal, raw_measured)
  
  write.csv(raw_combined, file_name)
  ...
  ggplot() +
    ...
    geom_line(data = models_combined,
              aes(x = Dose, y = fit, color = Source, linetype = Source),
              size = 1) +
    facet_wrap(~ Marker, scales = "free_y", nrow = 1) +
    scale_x_log10() +
    scale_color_manual(name = "Plasma", values = color_names) +
    labs(
      title = paste(chemical, ": Nominal vs Measured Dose–Response (LL.4)"),
...
# Example usage for PFOS
pfos_plot <- create_drc_plot(
  data_combined, "PFOS",
  "../invivo-pfas-immunotox/R input 56d/tcplfit_input_PFOS.csv",
  c("Measured PFOS" = "#8405a7", "Nominal Dose" = "#f68f44")
)
ggsave("../invivo-pfas-immunotox/R input 56d/plots/pfos_drc.png", pfos_plot, width = 9, height = 3, dpi = 300, bg = "white")

# Example usage for PFOA
pfoa_plot <- create_drc_plot(
  data_combined, "PFOA",
  "../invivo-pfas-immunotox/R input 56d/tcplfit_input_PFOA.csv",
  c("Measured PFOA" = "#8405a7", "Nominal Dose" = "#f68f44")
)
ggsave("../invivo-pfas-immunotox/R input 56d/plots/pfoa_drc.png", pfoa_plot, width = 9, height = 3, dpi = 300, bg = "white")
...
##Age
```{r}
data <- read.csv("../invivo-pfas-immunotox/R input 56d/age_29_56.csv")

expand_rows<- function(df, col, week_label){
  df %>%
    uncount(!!sym(col)) %>%
    mutate(Age = week_label)
}

# Expand for 6 weeks
data_6w <- expand_rows(data, "X6weeks", "6")
# Expand for 8 weeks
data_8w <- expand_rows(data, "X8weeks", "8")
# Combine both
df_long <- bind_rows(data_6w, data_8w)
df_long <- df_long %>% dplyr::select(-X6weeks, -X8weeks)
df_long$Age <- as.numeric(df_long$Age)
df_long <- df_long %>%
  separate(Dose, into = c("Chemical", "Dose_value"), sep = " ", convert = TRUE) %>%
  mutate(
    # if Dose_value is NA, use Chemical name instead
    Dose_value = ifelse(is.na(Dose_value), Chemical, Dose_value)
  )
# Optional: reorder Dose_value numerically
df_long <- df_long %>%
  mutate(Dose_value = factor(as.character(Dose_value)))

df_long <- df_long %>%
  mutate(Chemical = ifelse(!(Chemical %in% c("PFOS", "PFOA")), "PFOS", Chemical)) %>%
  bind_rows(
    df_long %>%
      filter(!(Chemical %in% c("PFOS", "PFOA"))) %>%
...
df_long <- df_long %>%
  mutate(Age = as.numeric(Age),
         Dose_value= factor(as.character(Dose_value), levels = c("Naïve","Vehicle", "0.166","0.5","1","1.5")))

# Vehicle medians
vehicle_medians <- df_long %>%
  filter(Dose_value == "Vehicle") %>%
  group_by(Exposure, Chemical, Dose_value) %>%
  summarise(vehicle_median = median(Age, na.rm = TRUE), .groups = "drop")

# Summarize data to mean and SD
...
# Bar plot with mean ± SD + individual points
plot <- ggplot() +
  # Individual points
   geom_jitter(data = df_long, aes(x = Dose_value, y = Age, color = Sex),
              size = 1.5, alpha = 0.6, width = 0.2, height = 0) +
  # Mean ± SD bars
  geom_bar(data = summary_data, aes(x = Dose_value, y = mean_age, fill = Dose_value),
           stat = "identity", color = "black", alpha = 0.4, width = 0.6,
           position = position_dodge(width = 0.7)) +
  geom_errorbar(data = summary_data,
                aes(x = Dose_value, ymin = mean_age - sd_age, ymax = mean_age + sd_age),
                width = 0.2, position = position_dodge(width = 0.7)) +
  # Facets and scales
  facet_wrap(Exposure ~ Chemical, scales = "free_y", nrow = 2) +
    scale_color_manual(name = "Sex", values = sex_colors) +
  scale_fill_manual(name = "Dose", values = custom_colors_named) +
  labs(title = "Individual Values with Mean ± SD by Dose and Sex", x = "Dose", y = "Age") +
...
print(plot)
ggsave("../invivo-pfas-immunotox/R input 56d/plots/age.png", plot, width = 10, height = 6, dpi =300)
replicate_summary <- df_long%>%
  group_by(Sex, Dose_value, Chemical, Exposure) %>%  
  summarise(n = n(), .groups = "drop")
min_n <- min(replicate_summary$n)
max_n <- max(replicate_summary$n)
cat(sprintf("n = %d–%d mice/group\n", min_n, max_n))
...
```{r}
library(tcplfit2) 
datatcplfit1 <- read.csv("../invivo-pfas-immunotox/R input 56d/tcplfit_input_PFOA.csv")
datatcplfit2 <- read.csv("../invivo-pfas-immunotox/R input 56d/tcplfit_input_PFOS.csv")
datatcplfit <- rbind(datatcplfit1, datatcplfit2)
```

###PFOA
```{r}
data_measured <- datatcplfit %>%
  filter(Source %in% c("Measured PFOA"))
data_measured <- data_measured %>%
  mutate(Dose = ifelse(Dose == "Vehicle", 0, as.numeric(Dose))) %>%
  mutate(Dose = as.numeric(Dose)) %>%
  filter(!is.na(Dose)) 

#I need a control so i made mean_measure =0 if dose is 0
data_measured <- data_measured %>%
  mutate(Mean_Measured = if_else(Dose == 0, 0, Mean_Measured))
# Cytokines to process
markers <- unique(data_measured$Marker) 

# Initialize result storage
out_results_pfoa<- list()
fit_results <- list()

chem <- "PFOA"
for (marker in markers) {
  message("=== Processing marker:", marker, "===")
  out_results_pfoa[[marker]] <- list()
  fit_results[[marker]] <- list()
  
    # Filter test chemical data for this marker
    test_chem <- data_measured %>%
      filter(PFAS == chem, Mean_Measured > 0, Marker == marker, !is.na(Value))
    
    # Summarize vehicle control for this marker
    vehicle_ctrl <- data_measured %>%
      filter(PFAS == chem, Mean_Measured == 0, Marker == marker) %>%
      summarise(
        Median = median(Value, na.rm = TRUE),
        nMad = mad(Value, constant = 1.4826, na.rm = TRUE)
      )
    ...
    if (nrow(test_chem) == 0 || all(is.na(test_chem$Value))) {
    ...
    row <- list(
      conc = as.numeric(test_chem$Mean_Measured),
      resp = test_chem$Value,
    ...
    out_results_pfoa[[marker]][[chem]] <- out
    ...
    # Plot fits
    tryCatch({
      plot1 <- plot_allcurves(fit, test_chem$Dose, test_chem$Value, log_conc = TRUE)
...
```{r}
data_measured <- datatcplfit %>%
  filter(Source %in% c("Measured PFOS"))
data_measured <- data_measured %>%
  mutate(Dose = ifelse(Dose == "Vehicle", 0, as.numeric(Dose))) %>%
  mutate(Dose = as.numeric(Dose)) %>%
  filter(!is.na(Dose)) 
...
data_measured <- data_measured %>%
  mutate(Mean_Measured = if_else(Dose == 0, 0, Mean_Measured))
data_measured$Mean_Measured <-as.numeric(data_measured$Mean_Measured)
data_measured$Value <-as.numeric(data_measured$Value)
# Cytokines to process
markers <- unique(data_measured$Marker) 
print(markers)
# Initialize result storage
out_results_pfos<- list()
fit_results <- list()
chem <- "PFOS"
for (marker in markers) {
  message("=== Processing marker:", marker, "===")
  out_results_pfos[[marker]] <- list()
  fit_results[[marker]] <- list()
    ...
    out_results_pfos[[marker]][[chem]] <- out
...
###Nominal
```{r}
data_nominal <- filter(datatcplfit, Source =="Nominal Dose")
data_nominal <- data_nominal %>%
  mutate(Dose = ifelse(Dose == "Vehicle", 0, as.numeric(Dose))) %>%
  mutate(Dose = as.numeric(Dose)) %>%
  filter(!is.na(Dose)) 
# Convert mg/kg/day to mg/kg (so its comparable)
data_nominal <- data_nominal %>%
  mutate(Dose = Dose*56)


chemicals <- unique(data_nominal$PFAS)
# Cytokines to process
markers <- unique(data_nominal$Marker) 
print(markers)
# Initialize result storage
out_results_nominal<- list()
fit_results <- list()

for (marker in markers) {
  message("=== Processing marker:", marker, "===")
  out_results_nominal[[marker]] <- list()
  fit_results[[marker]] <- list()
  ...
  for (chem in chemicals) {
    ...
    # Filter test chemical data for this marker
    test_chem <- data_nominal %>%
      filter(PFAS == chem, Dose > 0, Marker == marker, !is.na(Value))
    
    # Summarize vehicle control for this marker
    vehicle_ctrl <- data_nominal %>%
      filter(PFAS == chem, Dose == 0, Marker == marker) %>%
      summarise(
        Median = median(Value, na.rm = TRUE),
        nMad = mad(Value, constant = 1.4826, na.rm = TRUE)
      )
    ...
    out_results_nominal[[marker]][[chem]] <- out
...
nominal_ALP  <- extract_marker_data(out_results_nominal$ALP, "ALP") %>%
  mutate(Source = "nominal")
nominal_ALT  <- extract_marker_data(out_results_nominal$ALT, "ALT")%>%
  mutate(Source = "nominal")
nominal_CA  <- extract_marker_data(out_results_nominal$CA, "CA")%>%
  mutate(Source = "nominal")
nominal_TGL  <- extract_marker_data(out_results_nominal$TGL, "TGL")%>%
  mutate(Source = "nominal")

pfoa_ALP  <- extract_marker_data(out_results_pfoa$ALP, "ALP")%>%
  mutate(Source = "PFOA_measured_liver")
pfoa_ALT  <- extract_marker_data(out_results_pfoa$ALT, "ALT")%>%
  mutate(Source = "PFOA_measured_liver")
pfoa_CA  <- extract_marker_data(out_results_pfoa$CA, "CA")%>%
  mutate(Source = "PFOA_measured_liver")
pfoa_TGL  <- extract_marker_data(out_results_pfoa$TGL, "TGL")%>%
  mutate(Source = "PFOA_measured_liver")

pfos_ALP  <- extract_marker_data(out_results_pfos$ALP, "ALP") %>%
  mutate(Source = "PFOS_measured_liver")
pfos_ALT  <- extract_marker_data(out_results_pfos$ALT, "ALT") %>%
  mutate(Source = "PFOS_measured_liver")
pfos_CA  <- extract_marker_data(out_results_pfos$CA, "CA") %>%
  mutate(Source = "PFOS_measured_liver")
pfos_TGL  <- extract_marker_data(out_results_pfos$TGL, "TGL") %>%
  mutate(Source = "PFOS_measured_liver")

combined<- bind_rows(nominal_ALP, nominal_ALT, nominal_CA,nominal_TGL, pfoa_ALP,pfoa_ALT, pfoa_CA,pfoa_TGL, pfos_ALP,pfos_ALT, pfos_CA,pfos_TGL)

#getting dose info from original df Calculate dose range per Chemical–Source–Marker group
df <- datatcplfit %>%
  mutate(
    Dose_final = if_else(Source == "Nominal Dose", Cumulative_Dose, Mean_Measured)
  )

dose_ranges <- df %>%
  group_by(PFAS, Source, Marker) %>%
  summarise(
    minDose = min(Dose_final, na.rm = TRUE),
    maxDose = max(Dose_final, na.rm = TRUE),
    .groups = "drop"
  )%>%
  rename(PFAS ="Chemical")
# Apply filters
combined_filtered <- combined %>%
  left_join(dose_ranges, by = c("Chemical", "Source", "Marker")) %>%
  mutate(
    # Step 1: Check if BMD is within min/max range if available
    in_range = if_else(!is.na(minDose) & !is.na(maxDose),
                       BMD_uM >= minDose & BMD_uM <= maxDose,
                       TRUE),  # keep if range missing
    
    # Step 2: Hit threshold
    high_hit = hit >= 0.9,
    
    # Step 3: Valid BMDL
    valid_BMDL = !is.na(BMDL_uM),
    
    # Step 4: Ratio filter
    ratio_ok = (BMDU_uM / BMDL_uM) < 40,
    
    # Step 5: Combine all filters
    keep = in_range & high_hit & valid_BMDL & ratio_ok,
    
    # Step 6: Set invalid BMDs to NA
    BMD_uM  = ifelse(keep, BMD_uM, NA),
    BMDL_uM = ifelse(keep, BMDL_uM, NA),
    BMDU_uM = ifelse(keep, BMDU_uM, NA)
  ) %>%
  # Step 7: Remove rows where all BMD columns are NA
  filter(!is.na(BMD_uM) & !is.na(BMDL_uM) & !is.na(BMDU_uM))

p <- ggplot(combined_filtered, aes(x = BMD_uM, y = Chemical, color = Source)) +
  geom_point(position = position_dodge(width = 0.7), size = 3) +
  geom_errorbarh(
    aes(
      xmin = ifelse(is.na(BMDL_uM), BMD_uM, BMDL_uM),
      xmax = ifelse(is.na(BMDU_uM), BMD_uM, BMDU_uM)
    ),
    height = 0.3,
    position = position_dodge(width = 0.7)
  ) +
  facet_grid(~Marker) +
  scale_x_log10() +
  scale_color_manual(values = c("nominal" = "#7e03a8", "PFOS_measured_liver" = "#ed7953", "PFOA_measured_liver" = "#ed7953")) +
  labs(
    title = "Comparison of BMDs",
    x = "Benchmark Dose (mg/kg, log scale)",
    y = "Marker"
  ) +
  theme_bw() +
...
print(p)
ggsave("../invivo-pfas-immunotox/R input 56d/plots/BMD_Comparison_DotWhisker_filtered.png", p, width = 10, height = 6, dpi =300)
```
###Plot on original drc
```{r}

# For PFOA
bmd_lines_pfoa <- combined_filtered %>%
  filter(Chemical == "PFOA", Marker %in% c("ALP", "ALT"))

# For PFOS
bmd_lines_pfos <- combined_filtered %>%
  filter(Chemical == "PFOS", Marker %in% c("ALP", "ALT"))
pfos_plot <- pfos_plot +
  geom_vline(data = bmd_lines_pfos, aes(xintercept = BMD_uM),
             linetype = "dotted", color = "black", size = 0.8)

pfoa_plot <- pfoa_plot +
  geom_vline(data = bmd_lines_pfoa, aes(xintercept = BMD_uM),
             linetype = "dotted", color = "black", size = 0.8)
print(pfos_plot)
ggsave("../invivo-pfas-immunotox/R input 56d/plots/drc_pfos.png", pfos_plot, width = 10, height = 6, dpi =300)
print(pfoa_plot)
ggsave("../invivo-pfas-immunotox/R input 56d/plots/drc_pfoa.png", pfoa_plot, width = 10, height = 6, dpi =300)
```
