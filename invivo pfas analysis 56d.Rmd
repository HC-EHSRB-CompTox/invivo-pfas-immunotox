...
make_plot_by_sex <- function(data, summary, sex_label) {
  ...
  data <- data %>%
    mutate(Dose = factor(Dose, levels = c("Naive", "Vehicle", "0.166", "0.5", "1", "1.5")))

  ggplot(data, aes(x = Dose, y = Value, color = Marker)) +
    geom_jitter(
      size = 1, alpha = 0.4, width = 0.2
    ) +
    geom_boxplot(
      aes(group = interaction(Dose, Marker), fill = Marker),
      outlier.shape = NA, alpha = 0.2, color = "black", width = 0.5
    ) +
    ...
    labs(
      title = paste("Normalized Response (Boxplot + Vehicle Median) -", sex_label),
      x = NULL,
      y = NULL,
      color = "Marker",
      fill = "Marker"
...
# Convert all columns that might contain <MDL to character
  ...
  if (pfas_name %in% colnames(data_conc)) {
    for (dose_level in unique(mdls_df$Group)) {
      ...
      if (length(mdl_val) == 1 && !is.na(mdl_val)) {
        # Extract dose part from Group (e.g., "PFOA 0.166" → "0.166")
...
data_conc_subset <- data_conc %>%
  select(Mouse.ID, Sex, Group, all_of(pfas_cols))
...
print(p)
ggsave(filename = "../invivo-pfas-immunotox/R input 56d/plots/plasma_conc.png", plot = p, width = 10, height = 5, dpi = 300,bg = "white")
```
...
#Epected v real
```{r}
dose_levels <- c("0.166", "0.5", "1", "1.5")

#Compute average body weight across timepoints (Baseline to Week8)
avg_bw <- data_bw %>%
  filter(Dose %in% dose_levels, Marker %in% c("Baseline", paste0("Week", 1:8))) %>%
  group_by(Sample., Sex, PFAS, Dose) %>%
  summarise(Avg_BW_g = mean(Value, na.rm = TRUE), .groups = "drop") %>%
  mutate(Avg_BW_kg = Avg_BW_g / 1000)  # convert g to kg

#Calculate total dose received over 56 days
avg_bw <- avg_bw %>%
  mutate(
    Dose_mg_per_kg_per_day = as.numeric(as.character(Dose)),
    Total_dose_mg = Avg_BW_kg * Dose_mg_per_kg_per_day * 56
  )


# View result
head(avg_bw)
data_conc_change <- data_conc %>%
  rename(`Sample.` = `Mouse.ID`)
joined_df <- avg_bw %>%
  left_join(data_conc_change %>% select(Sample., PFOA, PFOS), by = "Sample.") %>%
  mutate(
    PFOS_total_mg = as.numeric(PFOS) * Avg_BW_kg,
    PFOA_total_mg = as.numeric(PFOA) * Avg_BW_kg
  )


# PFOS
pfos <-ggplot(
  joined_df %>% filter(PFAS == "PFOS"),
  aes(x = PFOS_total_mg, y = Total_dose_mg, color = Dose)
) +
  # points colored by Dose
  geom_point(size = 2, alpha = 0.8) +
  # single 1:1 identity line
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray50") +
  # one overall regression line
  geom_smooth(
    aes(group = 1),       # override grouping so only one trend
    method = "lm",
    se = FALSE,
    color = "black"
  ) +
  ...
  labs(
    x = "Administered Total PFOS (mg)",
    y = "Observed Total PFOS (mg)",
    color = "Dose (mg/kg/day)",
    title = "Observed vs. Administered Total PFOS per Mouse"
  ) +
...
ggsave(filename = "../invivo-pfas-immunotox/R input 56d/plots/pfos_rve.png", plot = pfos, width = 7, height = 5, dpi = 300, bg = "white")
# PFOA
p <-ggplot(
  joined_df %>% filter(PFAS == "PFOA"),
  aes(x = PFOA_total_mg, y = Total_dose_mg, color = Dose)
) +
  # points colored by Dose
  geom_point(size = 2, alpha = 0.8) +
  # single 1:1 identity line
  ...
  scale_color_viridis_d(option = "plasma") +
  labs(
    x = "Administered Total PFOA (mg)",
    y = "Observed Total PFOA (mg)",
    color = "Dose (mg/kg/day)",
    title = "Observed vs. Administered Total PFOA per Mouse"
  ) +
  theme_minimal(base_size = 14) +
  theme(
    panel.grid.minor = element_blank(),
...
ggsave(filename = "../invivo-pfas-immunotox/R input 56d/plots/pfoa_rve.png", plot = p, width = 7, height = 5, dpi = 300, bg = "white")
```
#Water consumption
```{r}
# Read and preprocess data
